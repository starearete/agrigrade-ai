...
-- Description: Deduplicate market records with identical name and district to prevent duplicate rendering

-- Temporary table to hold canonical (lowest id) mapping for duplicates
CREATE TEMPORARY TABLE canonical_markets AS
SELECT MIN(id) AS canonical_id, name, district
FROM markets
...
JOIN canonical_markets cm ON m.name = cm.name AND m.district = cm.district
...
LEFT JOIN canonical_markets cm ON m.id = cm.canonical_id
