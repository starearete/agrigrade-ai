...
public class MarketplaceListingService {
    ...
    public MarketplaceListingResponse createListing(String userPublicId, Long batchId, CreateListingRequest request) {
        ...
        boolean hasPhoto = images.stream().anyMatch(img -> "PHOTO".equalsIgnoreCase(img.getMediaType()));
        if (!hasPhoto) {
        ...
        Number completedAnalyses = (Number) entityManager.createNativeQuery(
                "SELECT COUNT(*) FROM ai_analyses WHERE batch_id = :batchId AND status = 'COMPLETED'"
        ).setParameter("batchId", batchId).getSingleResult();

        if (completedAnalyses == null || completedAnalyses.longValue() == 0) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "BATCH_NOT_VERIFIED", "Batch has not completed AI inspection. Please run AI analysis before listing.");
        }

        Number issuedCertificates = (Number) entityManager.createNativeQuery(
                "SELECT COUNT(*) FROM ai_certificates WHERE batch_id = :batchId AND status = 'ISSUED'"
        ).setParameter("batchId", batchId).getSingleResult();

        if (issuedCertificates == null || issuedCertificates.longValue() == 0) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "CERTIFICATE_NOT_VERIFIED", "Batch does not have an issued AI digital certificate.");
        }
        ...
        BigDecimal askingPrice = request.askingPricePerUnit() != null ? request.askingPricePerUnit() : new BigDecimal("25.00");
        BigDecimal minOrderQty = request.minimumOrderQuantity() != null ? request.minimumOrderQuantity() : new BigDecimal("100.00");
        ...
        // Lock associated AI analyses
        try {
            entityManager.createNativeQuery("UPDATE ai_analyses SET inspection_locked = TRUE, locked_at = NOW() WHERE batch_id = :batchId")
