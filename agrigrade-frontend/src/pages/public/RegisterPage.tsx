...
import { Sprout, User, Mail, Phone, Lock, Building, ArrowRight } from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#FCFBF5] p-4 sm:p-6 text-[#17201A]">
      <div className="max-w-md w-full bg-white border border-[#C5E6CC] rounded-3xl p-6 sm:p-8 shadow-sm animate-fade-in">
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-extrabold text-[#1B5E20]">{t('auth.registerTitle')}</h2>
          <p className="text-xs text-[#526158] mt-1">{t('auth.registerSubtitle')}</p>
        </div>

        {/* Role Toggle */}
        <div className="grid grid-cols-2 gap-2 bg-[#F4FAF4] border border-[#C5E6CC] p-1.5 rounded-2xl mb-6 text-xs font-bold">
          <button
            type="button"
            onClick={() => setRole('FARMER')}
            className={`py-2 rounded-xl transition-all ${
              role === 'FARMER' ? 'bg-[#2E7D32] text-white shadow-xs' : 'text-[#17201A] hover:bg-[#EEF8F0]'
            }`}
          >
            {t('auth.farmer')}
          </button>
          <button
            ...
            className={`py-2 rounded-xl transition-all ${
              role === 'BUYER' ? 'bg-[#2E7D32] text-white shadow-xs' : 'text-[#17201A] hover:bg-[#EEF8F0]'
            }`}
          >
            {t('auth.buyer')}
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
          <div>
            <label className="block font-bold text-[#17201A] mb-1">{t('auth.fullName')} *</label>
            <div className="relative">
              <User className="w-4 h-4 text-[#526158] absolute left-3 top-3" />
              <input
                type="text"
                required
          ...
          <div>
            <label className="block font-bold text-[#17201A] mb-1">{t('auth.mobileNumber')} *</label>
            <div className="relative">
              <Phone className="w-4 h-4 text-[#526158] absolute left-3 top-3" />
              <input
                type="tel"
                required
          ...
          <div>
            <label className="block font-bold text-[#17201A] mb-1">{t('auth.email')}</label>
            <div className="relative">
          ...
          {role === 'BUYER' && (
            <div>
              <label className="block font-bold text-[#17201A] mb-1">{t('auth.businessName')} *</label>
              <div className="relative">
                <Building className="w-4 h-4 text-[#526158] absolute left-3 top-3" />
                <input
                  type="text"
                  required
          ...
          <div>
            <label className="block font-bold text-[#17201A] mb-1">{t('auth.password')} *</label>
            <div className="relative">
          ...
          <div>
            <label className="block font-bold text-[#17201A] mb-1">{t('auth.confirmPassword')} *</label>
            <div className="relative">
              <Lock className="w-4 h-4 text-[#526158] absolute left-3 top-3" />
              <input
                type="password"
          ...
          >
            {isLoading ? t('common.loading') : t('auth.registerButton')} <ArrowRight className="w-4 h-4" />
          </button>
        </form>
